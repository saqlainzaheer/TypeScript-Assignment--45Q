//Q 16:
const guests: string[] = ["Ali", "Hamza", "Zain"];
...
console.log(
  ...
  [...placesToVisit].sort().reverse().join(", ")
);
...
//Q 20
const favoriteMountains: string[] = ["Everest", "K2", "rakhaposhi"];
console.log("Favorite Mountains:", favoriteMountains.join(", "));

//Q 21
const objectsToStore: { name: string; category: string }[] = [
...
//Q 29
const favorite_fruits: string[] = ["bananas", "apples", "strawberries"];
...
//Q 30
const usernames: string[] = ["admin", "ali", "hamza", "zain", "sarah"];
...
//Q 32
const current_users: string[] = ["ali", "hamza", "hamas", "muddasar", "Ahmed"];
const new_users: string[] = ["ahmer", "hussain", "hamas", "momin", "Ahmed"];

for (const new_user of new_users) {
  if (
    current_users.map((u) => u.toLowerCase()).includes(new_user.toLowerCase())
  ) {
...
function describe_car(
  ...
  const car_info: { [key: string]: string } = {
